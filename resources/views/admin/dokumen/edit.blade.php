                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.dokumen.update', $dokuman) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="judul" id="judul" value="{{ old('judul', $dokuman->judul) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('deskripsi', $dokuman->deskripsi) }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label for="file_dokumen" class="block text-sm font-medium text-gray-700">Ganti File Dokumen <small>(PDF, DOCX, XLSX, PPTX, Max 5MB, kosongkan jika tidak ingin mengubah)</small></label>
                            ...
                            @if($dokuman->file_path)
                                <p class="text-xs text-gray-500 mt-2">File saat ini: <a href="{{ asset('storage/' . $dokuman->file_path) }}" target="_blank" class="text-blue-600 hover:text-blue-800">{{ $dokuman->file_nama }}</a></p>
                            @endif
                        </div>

                        <div class="mb-4">
                            <label for="tanggal_publikasi" class="block text-sm font-medium text-gray-700">Tanggal Publikasi (Opsional)</label>
                            <input type="date" name="tanggal_publikasi" id="tanggal_publikasi" value="{{ old('tanggal_publikasi', $dokuman->tanggal_publikasi ? $dokuman->tanggal_publikasi->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="inline-flex items-center">
                                ...
                                <input type="checkbox" name="is_active" value="1" class="form-checkbox h-5 w-5 text-gray-600" {{ $dokuman->is_active ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Update Dokumen</button>
                        </div>
                    </form>
                </div>
            </div>
