<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Foto Baru ke Album: ' . Str::limit($album->nama, 30)) }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.albums.photos.store', $album) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <label for="file_photo" class="block text-sm font-medium text-gray-700">Upload File Foto <small>(JPG, PNG, GIF, SVG, Max 2MB)</small></label>
                            <input type="file" name="file_photo" id="file_photo" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept="image/jpeg,image/png,image/gif,image/svg+xml" required>
                            @error('file_photo')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="judul" id="judul" value="{{ old('judul') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Simpan Foto</button>
                        </div>
                    </form>
                </div>
            </div>
