        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 sm:p-8 text-gray-900">
                    <form action="{{ route('admin.indikator-kinerja.update', $indikatorKinerja) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <select id="sasaran_strategis_id" name="sasaran_strategis_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    ...
                                    @foreach($sasaranStrategis as $sasaran)
                                        <option value="{{ $sasaran->id }}" {{ old('sasaran_strategis_id', $indikatorKinerja->sasaran_strategis_id) == $sasaran->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <textarea id="nama_indikator" name="nama_indikator" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('nama_indikator', $indikatorKinerja->nama_indikator) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="satuan" name="satuan" type="text" class="mt-1 block w-full" :value="old('satuan', $indikatorKinerja->satuan)" required placeholder="Contoh: Ton, %, Izin, Rp Milyar" />
                                    <x-input-error :messages="$errors->get('satuan')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="urutan" :value="__('Nomor Urutan')" />
                                    <x-text-input id="urutan" name="urutan" type="number" class="mt-1 block w-full" :value="old('urutan', $indikatorKinerja->urutan)" required />
