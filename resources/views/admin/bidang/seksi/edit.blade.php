<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Seksi: ') . $seksi->nama_seksi . ' (Bidang: ' . $bidang->nama . ')' }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.bidang.seksi.update', [$bidang->id, $seksi->id]) }}" method="POST">
                        @csrf
                        @method('PUT') {{-- Gunakan metode PUT untuk update --}}

                        <div class="mb-4">
                            <x-input-label for="nama_seksi" :value="__('Nama Seksi')" />
                            <x-text-input id="nama_seksi" class="block mt-1 w-full" type="text" name="nama_seksi" :value="old('nama_seksi', $seksi->nama_seksi)" required autofocus />
                            <x-input-error :messages="$errors->get('nama_seksi')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="tugas" :value="__('Tugas Seksi')" />
                            <textarea name="tugas" id="tugas" class="tinymce-editor block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('tugas', $seksi->tugas) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="pejabat_kepala_id" id="pejabat_kepala_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach ($pejabats as $pejabat)
                                    <option value="{{ $pejabat->id }}" {{ old('pejabat_kepala_id', $seksi->pejabat_kepala_id) == $pejabat->id ? 'selected' : '' }}>{{ $pejabat->nama }} ({{ $pejabat->jabatan }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="urutan" class="block mt-1 w-full" type="number" name="urutan" :value="old('urutan', $seksi->urutan)" />
                            <x-input-error :messages="$errors->get('urutan')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <label for="is_active" class="inline-flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_active', $seksi->is_active) ? 'checked' : '' }}>
