<x-app-layout>
                    ...
                    <div class="overflow-x-auto border border-gray-200 rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-gray-900">{{ $page->title }}</div></td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 font-mono flex items-center">
                                            /{{ $page->slug }}
                                            {{-- PERUBAHAN DI SINI: Menggunakan class dan data-attribute --}}
                                            <button data-slug="/{{ $page->slug }}" class="ml-2 text-gray-400 hover:text-blue-600 copy-slug-button" title="Salin slug">
                                                <i class="bi bi-clipboard"></i>
                                            </button>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $page->updated_at->isoFormat('D MMM YYYY, HH:mm') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <a href="{{ route('static-pages.show', $page->slug) }}" target="_blank" class="text-green-600 hover:text-green-900" title="Lihat Halaman"><i class="bi bi-eye-fill text-lg"></i></a>
                                        <a href="{{ route('admin.static-pages.edit', $page) }}" class="text-indigo-600 hover:text-indigo-900" title="Edit"><i class="bi bi-pencil-square text-lg"></i></a>
                                ...
                                @empty
                                <tr><td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada halaman statis yang ditemukan.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
    ...
    {{-- Blok <script> telah dihapus dari sini --}}
</x-app-layout>